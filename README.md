# Homework #4A – Aligning & counting reads for gene expression analysis

**DUE: 11:59PM, 15 November** 
...
For each of these steps, there are a number of different programs / approaches that can be strung together into an analysis “pipeline”. All have their advantages and drawbacks, but there’s no single objective “best” way to analyze these data. For simplicity, you'll all be using the "wicked fast" pseudo-aligner `salmon`, but we in Part B of the HW we will assign you a mix of DE analysis programs. For this homework, please read some of the documentation for the programs you'll be using and run the data through them. Record your commands and note the metrics and output of your pipeline at each stage of the process. Please annotate this extensively with your thoughts on this pipeline, guided (but not limited!) by the questions below.

Once homework has been submitted, we will collate and compare the results of your analyses and present them in a later class. We’ll discuss the results as a class, so as part of this assignment be prepared to discuss the particulars of your program, pros / cons, etc.
...
Please copy this document and change the name to `hw4a_answers_[LASTNAME].md`, and reply in the document as prompted. We'll also ask you to create and save several scripts to be pushed with your HW repo. Everything we want you to submit for HW4A is listed at the bottom of the document.
...
A _de novo_ transcriptome assembled by the authors for this project, cleaned and downsampled to only the longest isoform per gene. This file kindly shared with us by author Danielle DeLeo. NOTE: This file is NOT in your GitHub repo. More on this in Step 0.

A file linking each contig to a series of GO terms for functional enrichment analysis. This file kindly shared with us by author Danielle DeLeo. NOTE: This file is NOT in your GitHub repo. More on this in Step 0.

A pdf of the paper from which these data were taken: `DeLeo_&_BrackenGrissom_2020.pdf`

An empty slurm script called `hw4_slurm_wrapper.txt` with placeholder info. Feel free to use this wrapper as the starting point for the (several!) slurm submissions you will do in the course of this homework.

A python script called `counts_to_table.py` for making a table out of individual read count files.

## STEP 0: Get raw reads

Because of their size and other considerations, the sequence files, reference transcriptome, and annotation file you'll be using for this project aren't in the GitHub repo. While the sequence files are technically small enough to be allowed, they take up enough space that we don't want each of you making a separate copy on the HPC, where space is precious. (The files shared by the authors are not published, so this is also a way to share those with you privately.) Instead, these files are already on the HPC in a directory called `HW4_files` in the `collaboration` directory of the class directory (same place all your project directories are located; you should all have full access here).
...
To create a symbolic link to the raw samples, do this **from your HW4 directory on your personal space**:

`ln -s [FULL_PATH_TO_SEQUENCES] HW4_resources`
...
You should see a new "file" named `HW4_resources` in your HW3 directory. Try `ls HW4_resources`. You should see a list of 20 gzipped samples, plus `S_debilis_eye_assembly_clean.fasta` and `S_debilis_eye_assembly_clean_GO_annotation.fasta`. If you do - awesome! If not, double-check your path and try again (and maybe google around for more guidance on creating and using symbolic links).

Now you can use `HW4_resources` as if it were a directory with all the files in them. Basically, it points right to the single centralized repository and lets you use those files without having to duplicate them in your own space.
...
Set up a conda environment called `HW4` for this homework, and add all necessary programs **except where otherwise noted** to this environment as you go through the homework.
...
Write a bash loop to trim and clip all samples. Include command(s) to write a line reading "Now processing: "\[SAMPLENAME\]" to the logfile when each sample starts being processed. Embed it in a slurm script called `hw4_clean-reads_[LASTNAME].txt`, and run it on the HPC. Use the following parameters:
...
Push your `hw4_clean-reads_[LASTNAME].txt` slurm script to GitHub as part part of your finished homework.
...
Typically, you have to index your reference transcriptome (or genome). Use as a reference transcriptome `S_debilis_eye_assembly_clean.fasta`, which is you should be able to access through the symbolic link you set up in Step 0. What command(s) did you use to index the transcriptome for alignment?
...
Set up command(s) to align your trimmed reads to the indexed transcriptome. Write a bash loop to align all of your samples, and embed it in a slurm script called `hw4_align_[LASTNAME].txt` to run your alignment on Poseidon. Use the following slurm parameters:
...
Push your `hw4_align_[LASTNAME].txt` slurm script to GitHub as part part of your finished homework. 
...
At this point, create a yaml file of your current HW4 conda environment (while that environment is activated) like this:
`conda env export --from-history > hw4a_conda_[LASTNAME].yml`
...
Rename this file `hw4_counts_[LASTNAME].tsv`, and push it to GitHub as part of your finished homework.

Congratulations! You are ready to proceed to differential expression analysis in the next part of the homework.
...
1. `hw4a_answers_[LASTNAME].md`: An annotated copy of this readme file including your answers.
2. `hw4a_conda_[LASTNAME].yml`: yaml file of your genex conda environment
3. `hw4_clean-reads_[LASTNAME].txt`: slurm script for cleaning raw reads
4. `hw4_align_[LASTNAME].txt`: slurm script for aligning clean reads to your transcriptome
5. `hw4_counts_[LASTNAME].tsv`: contig-by-sample read count table
